...
class QueueHandler {
    ...
    async inspect() {
        try {
            ...
            if (jobs.length && jobs[0].data && memory > jobs[0].data.input.length && 32 >= jobs[0].data.input.length) {
    ...
    async stopInspection() {
        console.log('Stopping inspection on runner 32G...');
